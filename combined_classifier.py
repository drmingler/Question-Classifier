...
def read_entire_dataset():
    df = pd.read_csv("data.txt", encoding="latin-1", sep="\n", header=None)
...
class PerformDataSplit:
    ...
    def _split_train_val(self, test_size=0.1, shuffle_state=True):
    ...
    def perform_split_and_write(self):
        ...
        Train.to_csv(r"train.csv", index=None, sep=" ")
        Val.to_csv(r"val.csv", index=None, sep=" ")
...
class CNNModel(nn.Module):
    ...
    def forward(self, sentences, sentence_lengths=None):
        ...
        print("embedded.permute ", embedded.shape)
...
def train_bow(
        model, train_dataset, optimizer, criterion, embedding_matrix, dataset_size
):
    total_loss = 0.0
    total_acc = 0.0
...
def test_model(model, embedding_matrix, bow_model=False):
    ...
    for _, data in test_dataset.items():
        ...
        for sentence, sentence_length, label in zip(
            ...
            print(
                "\nSENTENCE ==> ",
                " ".join([reverse_vocab_dict.get(_id) or "" for _id in sentence_]),
            )
            print("PREDICTED LABEL ==> ", reverse_label_dict.get(prediction))
            print("CORRECT LABEL ==> ", reverse_label_dict.get(label_))
...
if __name__ == "__main__":
    ...
    for epoch in range(epochs):
        ...
        print(
            f"\tTrain Loss on epoch {epoch + 1}: {train_loss:.3f} | Train Acc: {train_acc * 100:.2f}%"
        )
        print(
            f"\t Val. Loss on epoch {epoch + 1}: {valid_loss:.3f} |  Val. Acc: {valid_acc * 100:.2f}%"
        )

    print(test_model(model, EMBEDDING_MATRIX, bow_model=True))
